...
class Fire:
    ...
    # Detection helper used by drones
    def burning_fraction_in_disc(self, x_px: float, y_px: float, r_px: float):
    ...
    # ---------- Metrics ----------
    def compute_metrics(self, m_per_px: float):
        """Global fire metrics (BURNING + BURNED across entire grid)."""
        c = self.cell
        W, H = self.GW, self.GH
        burning_cells = 0
        burned_cells = 0
        ...
        return {
            ...
            "scorched_area_ha": burned_m2  / 10_000.0,
            "footprint_area_ha": (burning_m2 + burned_m2) / 10_000.0
        }

    def compute_local_metrics(self, cx: float, cy: float, r_px: float, m_per_px: float):
        """Local metrics inside a disk centered at (cx,cy) of radius r_px."""
        c = self.cell; r2 = r_px * r_px
        ...
        for gy in range(gy0, gy1 + 1):
            ...
            for gx in range(gx0, gx1 + 1):
                px = gx * c + 0.5 * c
                dx = px - cy + (px - px)  # keep structure, then correct
                dx = px - cx
                if dx * dx + dy2 > r2:
                    continue
                idx = self._idx(gx, gy)
